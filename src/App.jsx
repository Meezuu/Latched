    ...
    <div style={{ background:"#08060f", borderBottom:"1px solid #1a1030",
      ...
      {variants.map(({id,L,label})=>(
        <div key={id} style={{ display:"flex", flexDirection:"column",
          ...
          <div style={{ fontFamily:"'Syne',sans-serif", fontSize:6.5,
  ...
  useEffect(() => {
    ...
    if (betaMode && problem?.holds) {
      ...
      problem.holds.forEach(hh => {
        ...
        if (color) {
          ...
          ctx.font = `700 ${Math.max(8,w*0.021)}px 'Syne',sans-serif`;
    ...
    if (hasActive) {
      ...
      entries.forEach(([label, color], i) => {
        ...
        ctx.font = `800 7px 'Syne', sans-serif`;
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end" }}>
        <span style={{ fontSize:9, color:T.text3, fontFamily:"'Syne',sans-serif", letterSpacing:"0.12em", textTransform:"uppercase" }}>Board Angle</span>
      ...
      <div style={{ display:"flex", gap:3 }}>
        {ANGLES.map(a => (
          <button key={a} onClick={() => onChange(a)} style={{
            ...
            fontSize:10, cursor:"pointer", fontFamily:"'Syne',sans-serif",
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        ...
        <span style={{ fontSize:10, color:T.text2, fontFamily:"'Syne',sans-serif" }}>{desc} · {value}°</span>
      </div>
    </div>
  );
}
...
function GradePill({ grade, sent, small }) {
  return (
    <span style={{
      fontFamily:"'Syne',sans-serif", fontSize:small?8:9, fontWeight:700,
...
function GymMap({ homeGym, onSetHomeGym, onSeeGymClimbs }) {
  ...
  const buildPopupHtml = useCallback((gym, isHome) => {
    ...
    const addrLine = addrText
      ...
      : `<div style="font-size:9px;color:#444;font-family:'Syne',sans-serif;margin-top:4px;" data-addr-id="${escHtml(String(gym.id))}">loading address…</div>`;

    // Build social links — only include ones with real URLs
    const linkStyle = "display:flex;align-items:center;gap:6px;padding:7px 10px;background:#1e1e1e;border:1px solid #2e2e2e;border-radius:4px;text-decoration:none;font-size:9px;font-family:'Syne',sans-serif;letter-spacing:0.06em;color:#aaa;flex:1;white-space:nowrap;";
    ...
    <div style="font-family:'Geist',sans-serif;color:#e8e8e8;background:#181818;border-radius:6px;overflow:hidden;width:296px;">
      <div style="padding:14px;border-bottom:1px solid #242424;">
        <div style="display:flex;align-items:flex-start;gap:12px;">
          ...
          <div style="flex:1;min-width:0;">
            ...
            <div style="font-size:8px;color:#444;font-family:'Syne',sans-serif;letter-spacing:0.08em;margin-top:2px;">@${escHtml(gym.username)}</div>
            ${addrLine}
          </div>
        </div>
      </div>
      ...
      <div style="padding:10px 14px 12px;display:flex;flex-direction:column;gap:5px;">
        <button data-action="set-home-gym"
          style="width:100%;padding:8px 0;background:${isHome?"rgba(168,85,247,0.18)":"transparent"};border:1px solid ${isHome?"rgba(168,85,247,0.5)":"#2a2a2a"};color:${isHome?"#a855f7":"#666"};border-radius:4px;font-size:8px;font-family:'Syne',sans-serif;letter-spacing:0.08em;cursor:pointer;font-weight:700;">
          ${isHome ? "▲ HOME GYM" : "SET AS HOME GYM"}
        </button>
        <button data-action="see-gym-climbs"
          style="width:100%;padding:8px 0;background:rgba(168,85,247,0.12);border:1px solid rgba(168,85,247,0.35);color:#9333ea;border-radius:4px;font-size:8px;font-family:'Syne',sans-serif;letter-spacing:0.08em;cursor:pointer;font-weight:700;">
          VIEW ALL CLIMBS →
        </button>
      </div>
    </div>`;
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:10, flex:1, overflow:"hidden", padding:"14px" }}>
      {homeGym && (
        <div style={{ background:"rgba(168,85,247,0.06)", border:`1px solid rgba(168,85,247,0.22)`, borderRadius:6, padding:"10px 13px", display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ fontSize:8, color:T.text3, fontFamily:"'Syne',sans-serif", letterSpacing:"0.07em", marginTop:2 }}>HOME · @{homeGym.username}</div>
          </div>
          <button onClick={() => onSetHomeGym(null)} style={{ background:"none", border:"none", color:T.text3, cursor:"pointer", fontSize:8, fontFamily:"'Syne',sans-serif", letterSpacing:"0.06em", flexShrink:0, padding:"4px 0" }}>CHANGE</button>
        </div>
      )}
      ...
      {search && filtered.length > 0 && (
        <div style={{ maxHeight:200, overflowY:"auto", display:"flex", flexDirection:"column", gap:4 }}>
            ...
            return (
              <button key={gym.id} onClick={() => flyTo(gym)} style={{
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:9, color:T.text3, fontFamily:"'Syne',sans-serif", marginTop:2 }}>@{gym.username}</div>
                </div>
          ...
          {filtered.length > 50 && (
            <div style={{ fontSize:9, color:T.text3, fontFamily:"'Syne',sans-serif", textAlign:"center", padding:"4px 0" }}>
      ...
      {!search && !loading && (
        <div style={{ fontSize:9, color:T.text3, fontFamily:"'Syne',sans-serif", letterSpacing:"0.1em", textAlign:"center" }}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    link.rel = "stylesheet";
    link.href = "https://fonts.googleapis.com/css2?family=Geist:wght@300;400;500;600;700;800;900&family=Syne:wght@800&display=swap";
  ...
  const btnPri = { background:T.white, border:"none", color:T.bg, borderRadius:4, padding:"8px 13px", cursor:"pointer", fontSize:10, fontFamily:"'Syne',sans-serif", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em" };
  const btnSec = { background:T.bg3, border:`1px solid ${T.border}`, color:T.text2, borderRadius:4, padding:"8px 11px", cursor:"pointer", fontSize:10, fontFamily:"'Syne',sans-serif", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em" };
      ...
      <div style={{ position:"sticky", top:0, zIndex:50, background:T.bg+"f2", backdropFilter:"blur(16px)", borderBottom:`1px solid ${T.border}`, padding:"8px 14px" }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr", alignItems:"center" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:9, justifyContent:"center" }}>
            ...
            <div>
              ...
              <div style={{ marginTop:3 }}>
                <span style={{ fontSize:9, color:T.text3, fontFamily:"'Syne',sans-serif", letterSpacing:"0.07em" }}>
          ...
          <div style={{ display:"flex", gap:5, justifyContent:"flex-end" }}>
            ...
            <button onClick={() => setLogOpen(true)} style={{ background:T.bg3, border:`1px solid ${T.border}`, color:T.text, borderRadius:4, padding:"5px 11px", fontSize:9, cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:"0.06em" }}>SESSION</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"flex", background:T.bg, borderBottom:`1px solid ${T.border}` }}>
        {TABS.map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            fontFamily:"'Syne',sans-serif", fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase",
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"14px 14px 20px", boxSizing:"border-box" }}>
            <div style={{ display:"flex", gap:3, marginBottom:10 }}>
              {[["community","COMMUNITY"],["mine","MY CLIMBS"]].map(([key,label]) => (
                <button key={key} onClick={() => setClimbSource(key)} style={{
                  ...
                  fontSize:9, fontFamily:"'Syne',sans-serif", fontWeight:700, textTransform:"uppercase",
              ...
              {climbSource==="community" && (() => {
                ...
                return (
                  <button onClick={()=>setFilterOpen(true)} style={{
                    ...
                    fontSize:9, fontFamily:"'Syne',sans-serif", fontWeight:700, flexShrink:0,
            ...
            {climbSource==="community" && (
              <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:8}}>
                {(gradeMin||gradeMax) && <span onClick={()=>{setGradeMin(null);setGradeMax(null);}} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,color:T.purple,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>{gradeMin||"V0"}–{gradeMax||"V15"} ✕</span>}
                {filterAngle!==null && <span onClick={()=>setFilterAngle(null)} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,color:T.purple,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>{filterAngle}° ✕</span>}
                {filterSent && <span onClick={()=>setFilterSent(null)} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,color:T.purple,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>{filterSent} ✕</span>}
                {filterMinAscents && <span onClick={()=>setFilterMinAscents(null)} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,color:T.purple,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>{filterMinAscents}+ ascents ✕</span>}
                {filterMinQuality && <span onClick={()=>setFilterMinQuality(null)} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,color:T.purple,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>★{filterMinQuality}+ ✕</span>}
                {classicsOnly && <span onClick={()=>setClassicsOnly(false)} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,color:T.purple,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>Classics ✕</span>}
                {filterSetter && <span onClick={()=>setFilterSetter(null)} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,color:T.purple,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>@{filterSetter} ✕</span>}
                {filterDateAfter && <span onClick={()=>setFilterDateAfter("")} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,color:T.purple,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>after {filterDateAfter} ✕</span>}
                {filterDateBefore && <span onClick={()=>setFilterDateBefore("")} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,color:T.purple,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>before {filterDateBefore} ✕</span>}
                {sortBy!=="quality" && <span onClick={()=>setSortBy("quality")} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:T.bg3,border:`1px solid ${T.border}`,color:T.text3,borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>sort: {sortBy} ✕</span>}
                {injuryLeft  && <span onClick={()=>setInjuryLeft(false)}  style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:"rgba(255,60,60,0.12)",border:"1px solid rgba(255,60,60,0.4)",color:"#ff6060",borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>✕ left hand injury</span>}
                {injuryRight && <span onClick={()=>setInjuryRight(false)} style={{fontSize:9,fontFamily:"'Syne',sans-serif",background:"rgba(255,60,60,0.12)",border:"1px solid rgba(255,60,60,0.4)",color:"#ff6060",borderRadius:999,padding:"3px 8px",cursor:"pointer"}}>✕ right hand injury</span>}
              </div>
            )}
            ...
            {climbSource === "community" && (
              <>
                {climbsLoading ? (
                  <div style={{textAlign:"center",padding:"40px 0",color:T.text3,fontSize:11,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em"}}>
                    LOADING CLIMBS…
                  </div>
                ) : (
                <div style={{ fontSize:8, color:T.text3, fontFamily:"'Syne',sans-serif", letterSpacing:"0.1em", marginBottom:8 }}>
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:4, flexWrap:"wrap" }}>
                          ...
                          {settings.showFeltGrade && feltGradeLog[climb.uuid] && (
                            <span style={{fontSize:8,fontFamily:"'Syne',sans-serif",color:T.text3,letterSpacing:"0.04em"}}>consensus {feltGradeLog[climb.uuid]}</span>
                          )}
                          {tracked?.sends > 0 && <span style={{fontSize:8,fontFamily:"'Syne',sans-serif",color:T.purple,background:T.purpleDim,border:`1px solid ${T.purpleBrd}`,borderRadius:999,padding:"1px 6px"}}>SENT {tracked.sends}✓</span>}
                          {tracked && !tracked.sends && <span style={{fontSize:8,fontFamily:"'Syne',sans-serif",color:T.text3,background:T.bg3,border:`1px solid ${T.border}`,borderRadius:999,padding:"1px 6px"}}>{tracked.attempts} ATT</span>}
                          {betaLog[climb.uuid] && <span style={{fontSize:8,fontFamily:"'Syne',sans-serif",color:"#3b82f6",background:"rgba(59,130,246,0.10)",border:"1px solid rgba(59,130,246,0.30)",borderRadius:999,padding:"1px 6px"}}>β BETA</span>}
                        </div>
                        <div style={{ fontSize:9, color:T.text3, fontFamily:"'Syne',sans-serif", display:"flex", gap:10, alignItems:"center" }}>
                          <span>{climb.angle}°</span>
                          <button onClick={e=>{e.stopPropagation();setSetterProfile(climb.setter);}} style={{background:"none",border:"none",color:T.text2,cursor:"pointer",padding:0,fontFamily:"'Syne',sans-serif",fontSize:9,textDecoration:"underline",textDecorationColor:T.text3,textUnderlineOffset:"3px"}}>@{climb.setter}</button>
                      ...
                      <div style={{ marginTop:12, paddingTop:12, borderTop:`1px solid ${T.border}` }}>
                        ...
                        <div style={{ display:"flex", gap:6, marginTop:10 }}>
                          <button onClick={e=>{e.stopPropagation();setBoardProblem(climb);setBoardModalOpen(true);}}
                            style={{flex:1,background:T.bg3,border:`1px solid ${T.border}`,color:T.text,borderRadius:R,padding:"8px",cursor:"pointer",fontSize:10,fontFamily:"'Syne',sans-serif",letterSpacing:"0.08em"}}>
                            VIEW ON BOARD →
                          </button>
                ...
                {communityPage < filteredCommunity.length && (
                  <button onClick={() => setCommunityPage(p=>p+30)}
                    style={{width:"100%",background:T.bg2,border:`1px solid ${T.border}`,color:T.text2,borderRadius:R,padding:"10px",cursor:"pointer",fontSize:10,fontFamily:"'Syne',sans-serif",letterSpacing:"0.08em",marginTop:4}}>
            ...
            {climbSource === "mine" && (() => {
              ...
              return totalMine === 0 ? (
                <div style={card({textAlign:"center",padding:"32px 16px"})}>
                  <div style={{fontSize:10,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:8}}>NO PERSONAL CLIMBS YET</div>
              ...
              {dbMine.length > 0 && (
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:8}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:4,flexWrap:"wrap"}}>
                        ...
                        {settings.showFeltGrade && feltGradeLog[c.uuid] && (
                          <span style={{fontSize:8,fontFamily:"'Syne',sans-serif",color:T.text3,letterSpacing:"0.04em"}}>consensus {feltGradeLog[c.uuid]}</span>
                        )}
                        <span style={{fontSize:8,color:T.purple,fontFamily:"'Syne',sans-serif",border:`1px solid ${T.purpleBrd}`,borderRadius:999,padding:"1px 6px"}}>TB2</span>
                      </div>
                      <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",display:"flex",gap:10}}>
                        <span>{c.angle}°</span>
                    ...
                    <div style={{marginTop:12,paddingTop:12,borderTop:`1px solid ${T.border}`}}>
                      ...
                      <div style={{display:"flex",gap:6,marginTop:10}}>
                        <button onClick={e=>{e.stopPropagation();setBoardProblem(c);setBoardModalOpen(true);}}
                          style={{flex:1,background:T.bg3,border:`1px solid ${T.border}`,color:T.text,borderRadius:R,padding:"8px",cursor:"pointer",fontSize:10,fontFamily:"'Syne',sans-serif"}}>
                          VIEW ON BOARD →
                        </button>
              ...
              {localMine.length > 0 && dbMine.length > 0 && (
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",margin:"12px 0 8px"}}>
                  ...
                  {selectedClimb?.id===p.id && (
                    <div style={{marginTop:12,paddingTop:12,borderTop:`1px solid ${T.border}`}}>
                      ...
                      <button onClick={e=>{e.stopPropagation();setBoardProblem(p);setBoardModalOpen(true);}}
                        style={{marginTop:8,width:"100%",background:"transparent",border:`1px solid ${T.border}`,color:T.text3,borderRadius:R,padding:"7px",cursor:"pointer",fontSize:10,fontFamily:"'Syne',sans-serif",letterSpacing:"0.08em"}}>
                        VIEW ON BOARD →
                      </button>
                    </div>
                  )}
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"14px 14px 20px", boxSizing:"border-box" }}>
            ...
            <div style={{...card(), marginBottom:12, position:"relative"}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:14}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  {homeGym && (
                    <div style={{fontSize:9,color:T.purple,fontFamily:"'Syne',sans-serif",letterSpacing:"0.08em",marginBottom:4}}>
              ...
              <div style={{borderTop:`1px solid ${T.border}`,marginTop:14,paddingTop:12,display:"flex",gap:0}}>
                {[["SENDS",totalSends],["ATTEMPTS",totalAtt],["CLIMBS",myProblems.length],["SESSIONS",sessions.length]].map(([l,v],i,arr) => (
                  <div key={l} style={{flex:1,textAlign:"center",borderRight:i<arr.length-1?`1px solid ${T.border}`:"none"}}>
                    ...
                    <div style={{fontSize:7,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginTop:3}}>{l}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
              <div style={card({textAlign:"center",padding:"12px 8px"})}>
                ...
                <div style={{fontSize:7,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginTop:4}}>SEND RATE</div>
              </div>
              <div style={card({textAlign:"center",padding:"12px 8px"})}>
                <div style={{fontFamily:"'Geist',sans-serif",fontWeight:800,fontSize:28,color:T.white,lineHeight:1}}>
                  {followedSetters.size}
                </div>
                <div style={{fontSize:7,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginTop:4}}>FOLLOWING</div>
              </div>
            </div>
            ...
            {(() => {
              ...
              if (events.length === 0) return (
                <>
                  <div style={card({textAlign:"center",padding:"24px 0",color:T.text3,fontSize:11,fontFamily:"'Syne',sans-serif",marginBottom:12})}>
              ...
              return (
                <>
                  <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:10}}>RECENT ACTIVITY</div>
                  {events.map((e,i) => (
                    <div key={i} style={{...card(),marginBottom:6,display:"flex",gap:12,alignItems:"flex-start"}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",marginTop:2}}>{e.sub}</div>
                      </div>
                    </div>
                  ))}
                  {settingsButton()}
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"14px 14px 20px", boxSizing:"border-box" }}>
            ...
            {(() => {
              ...
              return (
                <div style={{...card({marginBottom:14}), borderLeft:`3px solid ${T.purple}`}}>
                  <div style={{fontSize:9,color:T.purple,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:8}}>TODAY</div>
                  <div style={{display:"flex",gap:16}}>
                    <div>
                      ...
                      <div style={{fontSize:8,color:T.text3,fontFamily:"'Syne',sans-serif",marginTop:3}}>ATTEMPTS</div>
                    </div>
                    <div>
                      <div style={{fontFamily:"'Geist',sans-serif",fontWeight:800,fontSize:28,color:T.purple,lineHeight:1}}>{todaySess.sends||0}</div>
                      <div style={{fontSize:8,color:T.text3,fontFamily:"'Syne',sans-serif",marginTop:3}}>SENDS</div>
                    </div>
                  </div>
                </div>
              );
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
              {[["SENDS",totalSends],["ATTEMPTS",totalAtt],["SEND RATE",sendRate+"%"],["MY CLIMBS",myProblems.length]].map(([l,v]) => (
                <div key={l} style={card({textAlign:"center"})}>
                  ...
                  <div style={{fontSize:8,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginTop:4}}>{l}</div>
                </div>
              ))}
            </div>
            ...
            {myProblems.length > 0 && (
              <div style={card({marginBottom:12})}>
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:12}}>SEND PYRAMID</div>
                {GRADES.map(g => {
                  ...
                  return (
                    <div key={g} style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                      <span style={{width:26,fontSize:10,fontFamily:"'Syne',sans-serif",fontWeight:700,color:sent>0?T.white:T.text3}}>{g}</span>
                      ...
                      <span style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",width:28,textAlign:"right"}}>{sent}/{tot}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {myProblems.filter(p=>p.sends===0&&p.attempts>0).length > 0 && (
              <div style={card({marginBottom:12})}>
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:10}}>PROJECTS</div>
                {myProblems.filter(p=>p.sends===0&&p.attempts>0).sort((a,b)=>b.attempts-a.attempts).map(p => (
                  <div key={p.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                    ...
                    <span style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif"}}>{p.attempts}att</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em"}}>SESSION LOG</div>
            ...
            {sessions.length === 0 ? (
              <div style={card({textAlign:"center",padding:"20px"})}>
                <div style={{fontSize:11,color:T.text3,fontFamily:"'Syne',sans-serif"}}>No sessions yet</div>
              </div>
            ) : sessions.map((s,i) => (
              <div key={i} style={card({marginBottom:8})}>
                ...
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.08em",marginBottom:s.notes?5:0}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:100,display:"flex",alignItems:"flex-end"}}
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:T.bg2,borderRadius:`${R*2}px ${R*2}px 0 0`,border:`1px solid ${T.border}`,borderBottom:"none",padding:"20px 16px 36px",width:"100%",boxSizing:"border-box"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
              <div>
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:4}}>DATE</div>
                <input type="date" value={newSess.date} onChange={e=>setNewSess(s=>({...s,date:e.target.value}))} style={inp}/>
              </div>
              <div>
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:4}}>DURATION (MIN)</div>
                <input type="number" value={newSess.duration} onChange={e=>setNewSess(s=>({...s,duration:Number(e.target.value)}))} style={inp}/>
              </div>
            </div>
            <div style={{marginBottom:12}}>
              <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:6}}>FEEL</div>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setLogOpen(false)} style={{flex:1,background:T.bg3,border:`1px solid ${T.border}`,color:T.text,borderRadius:R,padding:"11px",cursor:"pointer",fontSize:11,fontFamily:"'Syne',sans-serif"}}>CANCEL</button>
      ...
      {createOpen && draftClimb && (
        <div style={{position:"fixed",inset:0,background:T.bg,zIndex:300,display:"flex",flexDirection:"column"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"14px 14px 48px"}}>
            ...
            {/* ② Name */}
            <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:5}}>NAME</div>
            ...
            {/* ④ Grade */}
            <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:6}}>GRADE</div>
            ...
            {/* ⑤ Description */}
            <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:5}}>DESCRIPTION</div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:T.bg2,border:`1px solid ${T.border}`,borderRadius:R,padding:"12px 14px"}}>
              <div>
                ...
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif"}}>Both hands allowed on top hold</div>
              </div>
          ...
          <div style={{padding:"12px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:10,background:T.bg2,flexShrink:0}}>
            ...
            <div style={{flex:1}}>
              {boardProblem ? (
                <>
                  ...
                  <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",marginTop:1,display:"flex",gap:5,alignItems:"center"}}>
                    ...
                    {boardProblem.setter && (
                      <button onClick={()=>setSetterProfile(boardProblem.setter)} style={{background:"none",border:"none",color:T.text2,cursor:"pointer",padding:0,fontFamily:"'Syne',sans-serif",fontSize:9,textDecoration:"underline",textDecorationColor:T.text3,textUnderlineOffset:"3px"}}>
                        @{boardProblem.setter}
                      </button>
                    )}
                  </div>
            ...
            {boardProblem && !boardProblem.uuid && !view3d && (
              <button onClick={()=>setEditMode(v=>!v)} style={{
                ...
                fontSize:9, cursor:"pointer", fontFamily:"'Syne',sans-serif", fontWeight:700,
            ...
            <div style={{flex:1,position:"relative"}}>
              <Suspense fallback={
                <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",
                  background:"#090909",color:T.text3,fontFamily:"'Syne',sans-serif",fontSize:10,letterSpacing:"0.1em"}}>
                  LOADING 3D…
                </div>
              }>
                <Board3D
          ...
          <div style={{flex:1,overflowY:"auto",padding:"14px"}}>
            ...
            {editMode && (
              <div style={{marginTop:10}}>
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:6}}>TAP HOLDS TO ASSIGN ROLE</div>
              ...
              <>
                ...
                {boardProblem.uuid && (
                  <div style={{marginTop:8}}>
                    <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:5}}>
                  ...
                  {betaLog[boardProblem.uuid] && (
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      {["left","right","match"].map(a => {
                        ...
                        return <span key={a} style={{fontSize:8,fontFamily:"'Syne',sans-serif",color:BETA_COLORS[a]}}>{count}{a[0].toUpperCase()}</span>;
                      })}
                      <button onClick={()=>clearBeta(boardProblem.uuid)} style={{background:"none",border:"none",color:T.text3,cursor:"pointer",fontSize:8,fontFamily:"'Syne',sans-serif",padding:"0 2px"}}>✕</button>
                    </div>
                  )}
                </div>
              </>
            ...
            {!editMode && boardProblem && betaMode && (
              <div style={{marginTop:12}}>
                ...
                <div style={{fontSize:8,color:T.text3,fontFamily:"'Syne',sans-serif",marginBottom:10}}>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",zIndex:400,display:"flex",alignItems:"flex-end"}}
          ...
          <div onClick={e=>e.stopPropagation()} style={{
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 16px 14px",borderBottom:`1px solid ${T.border}`,flexShrink:0}}>
              ...
              }} style={{background:"none",border:`1px solid ${T.border}`,color:T.text3,borderRadius:R,padding:"4px 10px",cursor:"pointer",fontSize:9,fontFamily:"'Syne',sans-serif"}}>
                CLEAR ALL
              </button>
            </div>
            ...
            <div style={{overflowY:"auto",flex:1,padding:"16px 16px 8px"}}>
              ...
              {/* SORT */}
              <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:7}}>SORT BY</div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:7}}>
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em"}}>GRADE RANGE</div>
                <div style={{fontSize:9,fontFamily:"'Syne',sans-serif",color:T.text2}}>
              ...
              <div style={{fontSize:8,color:T.text3,fontFamily:"'Syne',sans-serif",marginBottom:20}}>
                Tap to set start · tap again to set end
              </div>

              {/* ANGLE */}
              <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:7}}>ANGLE</div>
              ...
              {/* STATUS */}
              <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:7}}>MY STATUS</div>
              <div style={{display:"flex",gap:4,marginBottom:20}}>
                {[[null,"All"],["not-sent","Not Sent"],["sent","Sent ✓"],["project","Project"]].map(([v,l])=>(
                  <button key={String(v)} onClick={()=>setFilterSent(filterSent===v?null:v)} style={{
                    ...
                    fontFamily:"'Syne',sans-serif",fontWeight:700,textAlign:"center",
              ...
              <div style={{display:"flex",gap:12,marginBottom:20}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:7}}>MIN QUALITY</div>
                  <div style={{display:"flex",gap:3}}>
                    {[1,1.5,2,2.5,3].map(q=>(
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:7}}>MIN ASCENTS</div>
                  <div style={{display:"flex",gap:3}}>
              ...
              {/* DATE SET */}
              <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:7}}>DATE SET</div>
              <div style={{display:"flex",gap:10,marginBottom:20}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:8,color:T.text3,fontFamily:"'Syne',sans-serif",marginBottom:4}}>AFTER</div>
                  <input type="date" value={filterDateAfter} onChange={e=>setFilterDateAfter(e.target.value)}
                    style={{...inp,fontSize:11,padding:"7px 10px"}}/>
                </div>
                <div style={{flex:1}}>
                  <div style={{fontSize:8,color:T.text3,fontFamily:"'Syne',sans-serif",marginBottom:4}}>BEFORE</div>
              ...
              <div style={{marginBottom:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:4}}>
                  <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em"}}>INJURY FILTER</div>
                  {(injuryLeft||injuryRight) && (
                    <button onClick={()=>{setInjuryLeft(false);setInjuryRight(false);}} style={{background:"none",border:"none",color:"#ff6060",cursor:"pointer",fontSize:8,fontFamily:"'Syne',sans-serif",padding:0}}>CLEAR</button>
                  )}
                </div>
                <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",marginBottom:10}}>
                ...
                <div style={{display:"flex",gap:8,marginBottom:8}}>
                  {[["left","L",injuryLeft,setInjuryLeft],["right","R",injuryRight,setInjuryRight]].map(([side,label,active,setter])=>(
                    <button key={side} onClick={()=>setter(v=>!v)} style={{
                      ...
                      <div style={{fontSize:9,fontFamily:"'Syne',sans-serif",fontWeight:700,
                        color:active?"#ff4444":T.text3,letterSpacing:"0.08em"}}>
                        {label === "L" ? "LEFT" : "RIGHT"} HAND
                      </div>
                      <div style={{fontSize:8,fontFamily:"'Syne',sans-serif",
                ...
                <div style={{fontSize:8,color:T.text3,fontFamily:"'Syne',sans-serif",lineHeight:1.5}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:T.bg3,border:`1px solid ${T.border}`,borderRadius:R,padding:"13px 14px",marginBottom:8}}>
                <div>
                  ...
                  <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif"}}>Quality ≥ 3.0 · 200+ ascents</div>
                </div>
            ...
            <div style={{overflowY:"auto",flex:1,gap:5,display:"flex",flexDirection:"column"}}>
              {setterList
                ...
                <button key={s.name}
                  ...
                  <span style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif"}}>{s.count} climbs</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:500,display:"flex",alignItems:"flex-end"}}
          ...
          <div onClick={e=>e.stopPropagation()} style={{
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
              <div style={{flex:1}}>
                <div style={{fontSize:9,color:T.purple,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:3}}>SEND LOGGED ✓</div>
            ...
            <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:8}}>YOUR GRADE</div>
            ...
            <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:8}}>NOTES</div>
            <textarea
            ...
            {sendReview.problem.uuid && (
              <div style={{background:T.bg3,border:`1px solid ${T.border}`,borderRadius:8,padding:"14px",marginBottom:16}}>
                <div style={{fontSize:9,color:T.purple,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:4}}>HELP IMPROVE THE INJURY FILTER</div>
                ...
                {[["leftCrimps","Left hand"],["rightCrimps","Right hand"]].map(([key,label])=>(
                  <div key={key} style={{marginBottom:12}}>
                    <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.08em",marginBottom:6}}>{label.toUpperCase()}</div>
                    <div style={{display:"flex",gap:5}}>
                      {[0,1,2,3,4,5].map(n=>(
      ...
      {settingsOpen && (
        <div style={{position:"fixed",inset:0,background:T.bg,zIndex:700,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"20px 14px"}}>
            <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.12em",marginBottom:10}}>GRADES</div>
            <div style={card({marginBottom:8})}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
                <div>
                  <div style={{fontSize:13,color:T.text,fontWeight:600,marginBottom:3}}>Consensus Grade</div>
                  <div style={{fontSize:10,color:T.text3,fontFamily:"'Syne',sans-serif",lineHeight:1.5}}>Show community consensus grade alongside the setter grade on climb cards</div>
                </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",zIndex:700,display:"flex",alignItems:"flex-end"}}
          ...
          <div onClick={e=>e.stopPropagation()} style={{
            ...
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:20}}>
              ...
              <div>
                <label style={{display:"block",background:T.bg3,border:`1px solid ${T.border}`,color:T.text2,borderRadius:R,padding:"7px 14px",cursor:"pointer",fontSize:9,fontFamily:"'Syne',sans-serif",fontWeight:700,textTransform:"uppercase"}}>
                  UPLOAD PHOTO
                ...
                {draftProfile.avatarDataUrl && (
                  <button onClick={()=>setDraftProfile(p=>({...p,avatarDataUrl:""}))} style={{background:"none",border:"none",color:T.text3,cursor:"pointer",fontSize:9,fontFamily:"'Syne',sans-serif",marginTop:6,padding:0}}>
                    REMOVE PHOTO
                  </button>
                )}
              </div>
            ...
            {/* Username */}
            <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:5}}>USERNAME</div>
            ...
            {/* Bio */}
            <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.1em",marginBottom:5}}>BIO</div>
            ...
            <div style={{padding:"18px 16px 14px",borderBottom:`1px solid ${T.border}`,flexShrink:0}}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:9,color:T.text3,fontFamily:"'Syne',sans-serif",letterSpacing:"0.06em",marginTop:3}}>
                  ...
                  <div style={{display:"flex",gap:2,alignItems:"flex-end",height:32}}>
                    {activeGrades.map(g => (
                      <div key={g} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:6,color:T.text3,fontFamily:"'Syne',sans-serif",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"clip"}}>{g}</div>
                      </div>
                    ))}
                  </div>
                );
